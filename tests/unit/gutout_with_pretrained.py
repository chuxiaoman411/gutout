...
def generate_gutout_mask(threshold, mask):
    ...
    attention_threshold = np.min(
        mask) + threshold * (np.max(mask)-np.min(mask))
...
if __name__ == '__main__':
    ...
    if args.image_path is None:
        args.image_path = "./sample_imgs_cifar10/plane.png"
    ...
    gutout_mask = generate_gutout_mask(0.6, mask)
